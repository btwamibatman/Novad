...
| POST | `/api/documents/upload` | Uploads a PDF or TXT file and saves its metadata in the database. |
...
Important: `/summarize` works only after `/analyze`; otherwise the API returns `400 Document must be analyzed first`.
